...
from bot import Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                BUTTON_SIX_NAME, BUTTON_SIX_URL, VIEW_LINK, aria2, QB_SEED, \
                dispatcher, DOWNLOAD_DIR, download_dict, download_dict_lock, LOGGER
...
class MirrorListener(listeners.MirrorListeners):
    def __init__(self, bot, update, isZip=False, extract=False, isQbit=False, pswd=None):
...
def _mirror(bot, update, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None):
    ...
    listener = MirrorListener(bot, update, isZip, extract, isQbit, pswd)
    ...
    if bot_utils.is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            cmsg = f"Use /{BotCommands.CloneCommand} to clone Google Drive file/folder\n\n"
            sendMessage(cmsg, bot, update)
            return
